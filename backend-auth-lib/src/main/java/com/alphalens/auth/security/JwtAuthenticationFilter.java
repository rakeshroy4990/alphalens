...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        if (token == null || token.isEmpty()) {
            LOG.warn("jwt_auth_missing_token method={} path={}", request.getMethod(), request.getRequestURI());
    ...
    private void unauthorized(HttpServletResponse response, String message) throws IOException {
        ...
        String safe = message == null ? "You are not logged in. Please login." : message.replace("\"", "'");
        response.getWriter().write(
                "{\"message\":\"" + safe + "\",\"errorCode\":\"AUTH_UNAUTHORIZED\",\"code\":\"AUTH_UNAUTHORIZED\"}");
    }
